<?php 
defined('BASEPATH') OR exit('No direct script access allowed');
?>


<!-- Page Title -->
<title>Kemahasiswaan | STMIK Pranata Indonesia</title>



  <!-- Start main-content -->
  <div class="main-content">
    <!-- Section: inner-header -->
    <section class="inner-header divider parallax layer-overlay overlay-dark-5" data-bg-img="<?php echo base_url(); ?>assets/depan/images/home/slider/slider-3.jpg">
      <div class="container pt-70 pb-20">
        <!-- Section Content -->
        <div class="section-content">
          <div class="row">
            <div class="col-md-12">
              <h2 class="title text-white">Kemahasiswaan</h2>
              <ol class="breadcrumb text-left text-black mt-10">
                <li><a href="javascript:void(0)">Tentang</a></li>
                <li><a href="javascript:void(0)">Struktur PI</a></li>
                <li class="active text-gray-silver">Kemahasiswaan</li>
              </ol>
            </div>
          </div>
        </div>
      </div>
    </section>

    <!-- Section: Have Any Question -->
    <section>
      <div class="container">
        <div class="row">
          <div class="col-md-10 col-md-push-1">
            <div id="section-one" class="mb-50">
              <h3><b>Kemahasiswaan STMIK Pranata Indonesia</b></h3>
              <hr>
              <p class="mb-20">
              <p align="justify">
          <ol type="A">
            <h4><b><li>Umum</li></b></h4>
            <p align="justify">Dalam upaya menghasilkan lulusan pendidikan tinggi yang potensial perlu adanya pembinaan kegiatan kemahasiswaan dilingkungan Sekolah Tinggi dan akademi Pranata Indonesia.</p>
            <br>
            <h4><b><li>Maksud</li></b></h4>
            <p align="justify">Memberikan arah dan muatan kehidupan kemahasiswaan yang bertujuan untuk mewujudkan tercapainya tujuan pendidikan nasional.</p>
            <br>
            <h4><b><li>Tujuan</li></b></h4>
            <p align="justify">Memberikan wahana dan sarana pengembangan diri mahasiswa kearah peluasan wawasan dan peningkatan kecendikiawanan serta integritas kepribadian melalui penalaran dan keilmuan, pengembangan minat bakat, kegiatan kesejahteraan mahasiswa, bakti sosial mahasiswa sehingga mahasiswa mampu meningkatkan potensi dirinya agar tumbuh dan berkembang.</p>            
            <br>            
            <h4><b><li>Sasaran</li></b></h4>
            <p align="justify">Meningkatkan potensi mahasiswa yang berilmu, berketerampilan dan memiliki sikap terpuji dalam membangun dirinya keluarga, bangsa dan negaranya.</p>
            <p align="justify">Program pembinaan mahasiswa terdiri dari dua bagian diantaranya sebagai berikut:</p>
              <ol type="1">
                <h5><b><li>Intra Kurikuler</li></b></h5>
                <p align="justify">Kegiatan yang berhubungan dengan kegiatan keilmuan seperti perkuliahan, ujian, penugasan kuliah, Kuliah praktek kerja lapangan, kuliah pengenalan lapangan, kuliah umum, karya ilmiah dan lainnya.</p>
                <h5><b><li>Ekstra Kurikuler</li></b></h5>
                <p align="justify"> 
                  <ol type="a">
                    <b><li>Kegiatan penalaran dan keilmuan</li></b>
                    <p align="justify">Forum akademik dengan mengadakan seminar dengan pembicara dari dalam dan luar kampus, diskusi ilmiah, program pelatihan keilmuan,seni,bahasa,klub komputer (pelatihan komputer, membuat email,design web dll), club bahasa asing, seminar tentang peluang kerja, strategi wawancara,pengembangan diri dan kewirausahaan dll.</p>
                    <b><li>Pengembangan Minat dan keilmuan</li></b>
                    <p>Pentas seni, pekan olahraga, pecinta alam dan lingkungan dll</p>
                    <b><li>Kegiatan kesejahteraan mahasiswa</li></b>
                    <p>Pemberian beasiswa, kegiatan rohani dan jasmani,koperasi mahasiswa</p>
                    <li><b>Kegiatan bakti sosial mahasiswa</b></li>
                    <p align="justify">Santunan yatim piatu, kunjungan ke panti jompo, donor darah,kemah kerja bakti mahasiswa, penyelenggaraan sunatan masal,santunan bencana alam,pembuatan SIM kolektif, Membuat tong sampah dilingkungan kampus dan lainnya.</p>
                  </ol>
                </p>
                <br>
                <h5><b><li>Organisasi Kemahasiswaan</li></b></h5>
                <p>Organisasi kemahasiswaan di tingkat Sekolah Tinggi/Akademik terdiri dari : 
                  <ol type="a">
                    <li>Senat Mahasiswa Perguruan Tinggi (SMPT)</li>
                    <li>Badan Perwakilan Mahasiswa (BPM)</li>
                    <li>Himpunan Mahasiswa Jurusan (HMJ)</li> 
                  </ol>
                  <br>
                  <ol type="1">
                    <b><li>Senat Mahasiswa (SEMA) terdiri dari :</li></b>
                    <p align="justify"><b><i>Senat Mahasiswa (SEMA)</i></b> berkedudukan di tingkat Sekolah Tinggi/Akademik dan merupakan kelengkapan non struktural Sekolah Tinggi/Akademik. Senat Mahasiswa (SEMA) Komisariat berkedudukan di tingkat kampus dan merupakan kelengkapan non struktural kampus.</p>
                    <b><li>Badan Perwakilan Mahasiswa (BPM) terdiri dari :</li></b>
                    <ol type="a">
                      <li>Badan Perwakilan Mahasiswa (BPM) berkedudukan di tingkat Sekolah Tinggi/Akademi dan merupakan kelengkapan non struktural Sekolah Tinggi/Akademi</li>
                      <li>Badan Perwakilan Mahasiswa (BPM) Komisariat Kampus berkedudukan di tingkat kampus dan merupakan kelengkapan non struktural kampus</li>
                    </ol>                   
                    <b><li>Organisasi Kemahasiswaan ditingkat jurusan adalah Himpunan Mahasiswa Jurusan (HMJ) yang terdiri dari :</li></b>
                    <p align="justify"><b><i>Himpunan Mahasiswa Jurusan (HJM)</i></b> berkedudukan di tingkat jurusan dan merupakan kelengkapan non struktural jurusan Himpunan Mahasiswa Jurusan (HJM) Komisariat Kampus berkedudukan di tingkat dan merupakan kelengkapan non struktural jurusan pada kampus tertentu</p>
                  </ol>

                  <h5><b>Kedudukan, Tugas Pokok dan Fungsi</b></h5>

                  <ol type="A">
                    <h6><b><li>SEMA</li></b></h6>
                    <p align="justify"><ol type="1">
                      <li>SEMA berkedudukan di tingkat Sekolah Tinggi/Akademi dan merupakan kelengkapan non struktural.</li>
                      <li>SEMA mempunyai tugas pokok merencanakan dan melaksanakan ekstrakulikuler yang bersifat penalaran dan keilmuan sesuai dengan garis-garis besar program yang ditetapkan oleh BPM serta memberikan pendapat,usul dan saran kepada pimpinan terutama yang berkaitan dengan pelaksanaan fungsi dan pencapaian tujuan pendidikan tinggi.</li>
                      <li>SEMA berfungsi sebagai wahana untuk merencanakan dan melaksanakan serta pengembangan kegiatan ekstra kurikuler terutama yang bersifat penataran dan keilmuan.</li>
                    </ol></p>

                    <h6><b><li>BPM</li></b></h6>
                    <p align="justify"><ol type="1">
                      <li>BPM berkedudukan ditingkat Sekolah Tinggi/Akademi dan merupakan kelengkapan non struktural.</li>
                      <li>BPM mempunyai tugas pokok menetapkan garis-garis besar program mengevaluasi pelaksanaan program SEMA serta memberikan pendapat,usul dan saran kepada pimpinan Sekolah Tinggi/Akademik.</li>
                      <li>BPM berfungsi sebagai perwakilan mahasiswa untuk menampung dan menyalurkan apresiasi mahasiswa melalui penetapan garis-garis besar program SEMA.</li>
                    </ol></p>

                    <h6><b><li>HMJ</li></b></h6>
                    <p align="justify"><ol type="1">
                      <li>HMJ berkedudukan di tingkat jurusan dan merupakan kelengkapan non-struktural pada jurusan dan merupakan sistem foundation dari SEMA.</li>
                      <li>HMJ mempunyai tugas pokok menyelenggarakan kegiatan ekstrakulikuler yang bersifat penalaran dan keilmuan,penelitian dan pengabdian kepada masyarakat.</li>
                      <li>HMJ berfungsi sebagai wahana pelaksanaan kegiatan ekstrakurikuler yang bersifat penalaran dan keilmuan, penelitian dan pengabdian kepada masyarakat sesuai dengan program studi/jurusannya.</li>
                    </ol></p>
                  </ol>

                  <h5><b>Keanggotaan dan Kepengurusan</b></h5> 

                  <ol type="A">
                    <h6><b><li>SEMA</li></b></h6>
                    <ol type="1">
                      <li>Keanggotaan SEMA terdiri dari mahasiswa yang terdaftar dan aktif mengikuti pendidikan di Sekolah Tinggi/Akademi atau terdiri dari para pengurus organisasi mahasiswa yang sah.</li>
                      <li>Keanggotaan SEMA terdiri dari ketua merangkap anggota,sekretaris merangkap anggota dan anggota pengurus lainnya yang terpilih secara demokratis sesuai dengan ketentuan yang berlaku.</li>
                      <li>Tata kerja kepengurusan SEMA ditentukan oleh rapat anggota.</li>
                      <li>Masa kerja kepengurusan SEMA satu tahun dan ketua tidak dapat di pilih kembali pada masa kepengurusan berikutnya.</li>
                      <li>Tata tertib kepengurusan SEMA ditetapkan dan disahkan oleh Ketua/Direktur.</li>
                      <li>Dalam melaksanakan tugas dan fungsinya, pengurus SEMA bertanggung jawab kepada Ketua/Direktur & Puket 3/Pudir 3.</li>
                    </ol>

                    <h6><b><li>BPM</li></b></h6>
                    <ol type="1">
                      <li>Keanggotaan BPM terdiri dari mahasiswa yang terdaftar dan aktif mengikuti kegiatan pendidikan di Sekolah Tinggi/Akademik.</li>
                      <li>Kepengurusan BPM terdiri dari ketua merangkap anggota dan anggota pengurus lain yang terpilih secara demokratis sesuai dengan ketentuan yang berlaku yang terbagi dalam komisi-komisi.</li>
                      <li>Masa kerja kepengurusan BPM satu tahun dan ketua tidak dapat dipilih kembali pada masa kepengurusan berikutnya.</li>
                      <li>Tata tertib kepengurusan BPM ditetapkan oleh anggota.</li>
                      <li>Kepengurusan BPM ditetapkan dan disahkan oleh Ketua/Direktur.</li>
                      <li>Dalam melaksanakan tugas dan fungsinya, pengurus BPM bertanggung jawab kepada Ketua/Direktur & Puket 3/Pudir3.</li>
                    </ol>

                    <h6><b><li>HMJ</li></b></h6>
                    <ol type="1">
                      <li>Keanggotaan HMJ terdiri dari mahasiswa yang terdaftar dan aktif mengikuti kegiatan pendidikan di jurusan yang bersangkutan.</li>
                      <li>Kepengurusan HMJ terdiri dari ketua merangkap anggota, sekretaris merangkap anggota dan anggota lainnya yang terpilih secara demokratis sesuai dengan ketentuan yang berlaku.</li>
                      <li>Masa kerja pengurus HMJ satu tahun dan ketua tidak pilih kembali pada masa kepengurusan berikutnya.</li>
                      <li>Tata tertib kepengurusan HMJ ditetapkan oleh rapat pengurus.</li>
                      <li>Kepengurusan HMJ ditetapkan dan disahkan oleh Ketua/Direktur.</li>
                      <li>Dalam melaksanakan tugas pokok dan fungsinya,pengurus HMJ bertanggung jawab kepada Ketua/Direktur c.q. Ketua jurusan.</li>
                    </ol>
                  </ol>
                </p>                
              </ol>
          </ol> 
        </p>
            </div>
          </div>
        </div>
      </div>
    </section>

  </div>
  <!-- end main-content -->

